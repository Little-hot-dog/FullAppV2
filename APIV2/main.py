...
def update_pc_to_table(input_id: int, data: dict, db: Session):
    ...
    # Удаление старых записей для данного id_raw
    db.query(SystemInfo).filter(SystemInfo.id_raw == id_raw).delete()

    # Добавление новых записей
    for param, value in data.items():
          # Пропускаем id_raw, так как это не параметр
            if isinstance(value, list):
                value = str(value)
            sys_info = SystemInfo(id_raw=id_raw, host=data['host'], param=param, value=value)
            db.add(sys_info)
...
async def update_pc(id_raw: int, data: SystemInfoUpdate, db: Session = Depends(get_db)):
    try:
